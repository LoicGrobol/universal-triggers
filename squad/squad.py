...
def main():
    ...
    who_questions_dev, what_questions_dev, where_questions_dev, when_questions_dev, what_questions_dev, how_questions_dev, why_questions_dev, which_questions_dev, other_questions_dev = (
        [] for i in range(9)
    )
    ...
    span_start = trigger_init.split(" ").index(
        target_answer.split(" ")[0]
    )  # start of target_answer
    ...
    ignore_indices = (
        [0] * (span_start)
        + [1] * (span_end - span_start + 1)
        + [0] * (len(trigger_token_ids) - 1 - span_end)
    )
    ...
    for _ in range(100):
        ...
        squad_utils.get_accuracy_squad(
            model,
