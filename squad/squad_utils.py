...
def evaluate_batch_squad(model, batch, trigger_token_ids, vocab, span_start, span_end):
    ...
    for idx in range(len(batch["metadata"])):
        ...
        orig_original_passage.append(
            deepcopy(batch["metadata"][idx]["original_passage"])
        )
        ...
        batch["metadata"][idx]["original_passage"] = (
            " ".join(trigger_words) + " " + batch["metadata"][idx]["original_passage"]
        )
        batch["metadata"][idx]["passage_tokens"] = (
            trigger_words + batch["metadata"][idx]["passage_tokens"]
        )
...
def get_accuracy_squad(
    model, dev_dataset, vocab, trigger_token_ids, answer, span_start, span_end
):
    """
    ...
    for batch in lazy_groups_of(
        ...
        output_dict = evaluate_batch_squad(
            model, batch, trigger_token_ids, vocab, span_start, span_end
        )
...
def get_average_grad_squad(
    model, vocab, trigger_token_ids, dev_dataset, span_start, span_end
):
    """
    ...
    iterator.index_with(vocab)
    for batch in lazy_groups_of(
        iterator(dev_dataset, num_epochs=1, shuffle=True), group_size=1
    ):
        optimizer.zero_grad()
        ...
        loss.backward()
        if batch_count == 0:
            grads = torch.sum(utils.extracted_grads[0], dim=0).detach()[
...
def get_best_candidates_squad(
    ...
    """
    loss_per_candidate = get_loss_per_candidate_squad(
        0,
        model,
        trigger_token_ids,
        cand_trigger_token_ids,
...
def get_loss_per_candidate_squad(
    ...
    for cand_id in range(len(cand_trigger_token_ids[0])):
        ...
        for batch in lazy_groups_of(
            ...
            loss += (
                evaluate_batch_squad(
                    model, batch, temp_trigger_token_ids, vocab, span_start, span_end
                )["loss"]
                .cpu()
                .detach()
                .numpy()
