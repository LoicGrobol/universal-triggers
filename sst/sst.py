...
def main():
    ...
    # otherwise train model from scratch and save its weights
        ...
        iterator = BucketIterator(
            batch_size=32, sorting_keys=[("tokens", "num_tokens")]
        )
    ...
    embedding_weight = utils.get_embedding_weight(
        model
    )  # also save the word embedding matrix
    ...
    for batch in lazy_groups_of(
        iterator(targeted_dev_data, num_epochs=5, shuffle=True), group_size=1
    ):
        ...
        trigger_token_ids = utils.get_best_candidates(
            model, batch, trigger_token_ids, cand_trigger_token_ids
        )
